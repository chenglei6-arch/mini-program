 ...
 * selectProgressForUpdate 使用行锁，配合 DbGameProgressStore.loadStateForUpdate
 * 保证同一用户的并发事件串行处理。
 */
@Mapper
public interface GameProgressMapper {

    @Select("SELECT progress_json AS progressJson, completed, version FROM game_progress "
        + "WHERE user_id=#{userId} AND game_id=#{gameId} FOR UPDATE")
    java.util.Map<String, Object> selectProgressForUpdate(@Param("userId") long userId,
        @Param("gameId") String gameId);

    @Select("SELECT progress_json AS progressJson, completed, version FROM game_progress "
        + "WHERE user_id=#{userId} AND game_id=#{gameId}")
    java.util.Map<String, Object> selectProgress(@Param("userId") long userId, @Param("gameId") String gameId);
    ...
    @org.apache.ibatis.annotations.Update("UPDATE game_progress SET progress_json=#{progressJson}, "
    ...
    java.util.Map<String, Object> selectEvent(@Param("userId") long userId, @Param("gameId") String gameId,
