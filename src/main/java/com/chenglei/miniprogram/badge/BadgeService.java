...
public class BadgeService {
    ...
    private Map<String, Boolean> conditions(String userId) {
        ...
        int completedFrogs = completedFrogsCount(userId);
        int paperCuttingDone = paperCuttingCompleted(userId) ? 1 : 0;
        PAPER_FROG_REQUIREMENT.forEach((badgeId, required) -> conditions.put(badgeId,
            completedFrogs >= required || (required == 1 && paperCuttingDone == 1)));

        JsonNode story = storyState(userId);
        int completedRoutes = story.path("completedRoutes").size();
        conditions.put("story-listener", completedRoutes >= 1);
        conditions.put("story-inheritor", completedRoutes >= 2);
        conditions.put("story-guardian", story.path("finished").asBoolean(false));
    ...
    private int completedFrogsCount(String userId) {
        String state = store.loadState(userId, "frog-puzzle");
        return state == null ? 0 : readTree(state).path("completedFrogs").size();
    }

    private boolean paperCuttingCompleted(String userId) {
        String state = store.loadState(userId, "paper-cutting");
        return state != null && readTree(state).path("completed").asInt(0) >= 1;
    }

    private JsonNode storyState(String userId) {
        String state = store.loadState(userId, "story");
        return state == null ? JSON.createObjectNode() : readTree(state);
    }

    private List<String> quizCompletedLevels(String userId) {
        Map<String, Object> row = quizMapper.selectProgress(userId);
        if (row == null || row.get("completedLevelsJson") == null) return List.of();
        try {
            JsonNode node = JSON.readTree(String.valueOf(row.get("completedLevelsJson")));
            List<String> levels = new ArrayList<>();
            node.forEach(item -> levels.add(item.asText()));
            return levels;
        } catch (Exception e) {
            return List.of();
        }
